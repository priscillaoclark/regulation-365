  ...
  const fetchData = async (page: number) => {
    ...
    try {
      const response = await fetch(
        `/api/ea/occ?page=${page}&pageSize=${metadata.pageSize}`
      );
      if (!response.ok) {
          ...
          {data.map((record) => (
            ...
            >
              ...
              <div className="flex flex-wrap gap-4">
                ...
                {record.start_documents && (
                  <Link
                    href={record.start_documents}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="text-lime-500 hover:underline"
                  >
                ...
                {record.termination_documents && (
                  <Link
                    href={record.termination_documents}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="text-lime-500 hover:underline"
                  >
                    View Termination Documents
                  </Link>
                )}
              </div>
            </article>
